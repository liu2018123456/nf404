...
<div id="fake-body" >
    ...
    <div server-rendered="true" class="Usercenter" >
        ...
        <li class="SlideList-add col-3" id="add-PPT" >
            ...
            <label style="margin-top: 70px" for="SlideImport-input" data-balloon="将PPT转为PDF后再导入，可获得更高的还原度哦！" data-balloon-pos="right" data-balloon-length="medium" class="Button Button--secondary Button--outline">
...
<!--  ------------------------------------------------------------------------------------    -->
<script>
<%Object dataSlide[][] = DataBaseManager.unionQuery("select slideId,name from slides where userId = '" + s + "'order by slideId desc", 2);%>
<%if (dataSlide.length!=0) {
				for (int i = 0; i < dataSlide.length; i++) {%>addAPPT("#add-PPT","<%=(String) dataSlide[i][0]%>","<%=(String) dataSlide[i][1]%>");<%}
				%>
					var maxSlideId = "<%=(String) dataSlide[0][0]%>";
 					maxSlideId = parseInt(maxSlideId)+1;
 					var userId = "<%=s%>";
				<%
			}else{%> var maxSlideId ="100001";<%}%>
</script>
